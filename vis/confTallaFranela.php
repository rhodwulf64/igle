		...
		<div class="col-lg-12">
			<input type="hidden" id="temporal"/>
				<table class="table table-striped table-bordered table-hover"  border="1" >
					<thead>		
						<tr>
							<th colspan="2"><center>Configuracion de Tallas de Franelas</center></th>
						</tr>
					</thead>
				<tr colspan="2">
					<th>
						<center>
							<div class="form-group has-default" id="haf_listarTallaFranela" style="width:400px;"><div class="on-focus clearfix" style="position: relative;"><font class="control-label">Seleccione Tallas de Franela a Editar:</font><br><select name="f_listarTallaFranela" class="form-control" size="10" onchange="SeleccionaItem(this.value);" id="f_listarTallaFranela" value="">');

							echo utf8_decode($loFuncion->fncreateComboSelectConf("tallafranela", "","idTtallaFranela","", ' ',"","Talla", $selectTallaFranela,"", "", "")); 
							echo utf8_Decode('
							</select><div class="tool-tip  slideIn" id="ttipf_listarTallaFranela" style="display:none;"></div></div></div>
						</center>
					</th>
				</tr>
				<tr colspan="2">
	...
	<div class="container" style="margin-top:5px; padding:5px; min-height: auto; background: #FFFFFF; -webkit-box-shadow: 0px 2px 5px 2px #999; -moz-box-shadow: 2px 2px 5px #999;">
		...
		<tr>
			<th colspan="2"><center>
					...
					<input type="hidden" name="KcharSelector" value="tallafranela">
...
<script>
	...
	{	
		...
		function fpEncender()
		{
			...
			loF.f_listarTallaFranela.disabled=false;
			loF.f_descripcion.disabled=false;
		...
		function fpApagar()
		{
			...
			loF.f_listarTallaFranela.disabled=true;
		...
		function fpCancelar()
		{
			...
			loF.txtHay.value=0;
			loF.f_listarTallaFranela.value="";
		...
		{
			var valueIndex=loF.f_listarTallaFranela.selectedIndex;
			loF.f_descripcion.value=loF.f_listarTallaFranela.options[valueIndex].text;
			loF.KcodCombo.value=loF.f_listarTallaFranela.options[valueIndex].value;
			loF.f_descripcion.focus();
			...
			$.ajax({
		        ...
		        success: function(data){
					...
					{
							alert("Error en el Estatus del elemento.");
				...
				if(confirm("Desea Desactivar a "+loF.f_descripcion.value+"?"))
				{
					...
					$.ajax({
				        ...
				        success: function(data){
							...
							{
									fpCancelar();
									alert(loF.f_descripcion.value+" Desactivado");
							...
							{
									fpCancelar();
									alert("No se pudo Desactivar "+loF.f_descripcion.value+".");
									fpInicial();									
							}
				...
				if(confirm("Desea Reactivar a "+loF.f_descripcion.value+"?"))
				{
					...
					$.ajax({
				        ...
				        success: function(data){
							...
							{
									fpCancelar();
									alert(loF.f_descripcion.value+" Reactivado");
									document.location.reload();
							...
							{
									fpCancelar();
									alert("No se pudo Reactivar "+loF.f_descripcion.value+".");
