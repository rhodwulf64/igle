		...
		<div class="col-lg-12">
			<input type="hidden" id="temporal"/>
				<table class="table table-striped table-bordered table-hover"  border="1" >
					<thead>		
						<tr>
							<th colspan="2"><center>Configuracion de Iglesias</center></th>
						</tr>
					</thead>
				<tr colspan="2">
					<th>
						<center>
							<div class="form-group has-default" onclick="fpAvisaSeleccionar(document.getElementById(\'KcodCombo\').value);" id="haf_listarIglesias" style="width:400px;"><div class="on-focus clearfix" style="position: relative;"><font class="control-label">Iglesia a Editar:</font><br><select title="Debe presionar el boton \'Seleccionar\' antes de editar un elemento."name="f_listarIglesias" class="form-control" size="10" onblur="vCampoVacio(this.id);" onchange="SeleccionaItem(this.value);" id="f_listarIglesias" value="">');

							echo utf8_decode($loFuncion->fncreateComboSelectConf("iglesia", "","idTiglesia","", ' ',"","Nombre", $selectIglesia,"", "", "")); 
							echo utf8_Decode('
							</select><div class="tool-tip  slideIn" id="ttipf_listarIglesias" style="display:none;"></div></div></div>
						</center>
					</th>
				</tr>
				<tr colspan="2">
	...
	<div class="container" style="margin-top:5px; padding:5px; min-height: auto; background: #FFFFFF; -webkit-box-shadow: 0px 2px 5px 2px #999; -moz-box-shadow: 2px 2px 5px #999;">
		...
		<tr>
			<th colspan="2"><center>
					...
					<input type="hidden" name="KcharSelector" value="iglesia">
...
<script>
	...
	{	
		...
		function fpEncender()
		{
			...
			loF.f_listarIglesias.disabled=false;
			loF.f_descripcion.disabled=false;
		...
		function fpApagar()
		{
			...
			loF.f_listarIglesias.disabled=true;
		...
		{
			var valueIndex=loF.f_listarIglesias.selectedIndex;
			loF.f_descripcion.value=loF.f_listarIglesias.options[valueIndex].text;
			loF.KcodCombo.value=loF.f_listarIglesias.options[valueIndex].value;
			loF.f_descripcion.focus();
