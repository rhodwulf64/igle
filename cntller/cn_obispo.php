<?php
	session_start();
   	require_once("../clases/claObispo.php");
	...
	switch($lsOperacion)
   	{
		case "buscar":
			...
			if($lbEnc)
			{
				...
				$lsFechaInicioDiocesis=$loSacerdote->asFechaInicioDiocesis;
				$lsFechaFinDiocesis=$loSacerdote->asFechaFinDiocesis;
		...
		case "buscar_like":
			...
			$cabecera = "<tr>
							...
							<td>Apellido</td>
							<td>Teléfono</td>
							<td>Fecha Inicio en Diócesis</td>
							<td>-</td>
						</tr>";
			for($i=0;$i<count($arr);$i++){
				$line.="<tr>";
					...
					$line.="<td>".$arr[$i]['FechaInicioDiocesis']."</td>";
					$line.="<td>
    ...
    function fpPasar_Campos($poPersona,$poFuncion,$lsGradoEstudio,$lsTallaFranela,$lsNacionalidad,$lsCedula)
    {
		...
		$lsFechaInicioDiocesis=$_POST["f_fechaInicioDiocesis"];
		$lsFechaFinDiocesis=$_POST["f_fechaFinDiocesis"];
		...
		$poPersona->asFechaInicioDiocesis=$lsFechaInicioDiocesis;
		$poPersona->asFechaFinDiocesis=$lsFechaFinDiocesis;
