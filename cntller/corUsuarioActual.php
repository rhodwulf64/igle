<?php
  ...
  if(!array_key_exists("usuario", $_SESSION)) 
	...
	$loPersona->asIDpersona=$_SESSION["IDTpersona"];
	$loPersona->asNacionalidad=$lsNacionalidad;
	$loUsuario->fpSetUsuario($_SESSION["usuario"]);
	$loUsuario->fpSetIDTusuario($_SESSION["IDTusuario"]);
	$lsCedula=$lsCeduUser;
	...
	switch($lsOperacion)
   	{
		...
		case "buscar":
			...
			if($lbEnc > 0)
			{
				...
				$lsIDTpersona=$loUsuario->fsGetIDFpersonas();
...
echo json_encode( $json );
    ...
    function fpPasarCamposUser($poUsuario,$lsNacionalidad,$lsCeduUser)
    {
		...
		$poUsuario->fpSetIDFpersonas($_SESSION["IDTpersona"]);
		$poUsuario->fpSetIDTusuario($_SESSION["IDTusuario"]);
		$poUsuario->fpSetRol('4');
    ...
    function fpPasarCamposPersonas($poPersona,$poFuncion)
    {
		...
		$poPersona->asIDpersona=$_SESSION["IDTpersona"];
		$poPersona->asIDTusuario=$_SESSION["IDTusuario"];
    }
?>
